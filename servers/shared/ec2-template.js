...
/**
 * Khởi tạo và chạy một EC2 server
 * @param {Object} config
 * @param {number} config.port        - Cổng lắng nghe (3001, 3002, 3003)
 * @param {string} config.serverId    - ID server ('ec2-1', 'ec2-2', 'ec2-3')
 * @param {string} config.serverName  - Tên hiển thị ('EC2-1', 'EC2-2', 'EC2-3')
 * @param {Object} config.instance    - Thông tin instance (ip, domain, region, zone, type, accent, accentRGB)
 */
module.exports = function createEC2Server(config) {
  const { port: PORT, serverId: SERVER_ID, serverName: SERVER_NAME, instance } = config;

  // Ghép tên + id vào object instance để buildHTML dùng
  const INSTANCE = { name: SERVER_NAME, id: SERVER_ID, ...instance };

  const app = express();

  // ── Cấu hình kết nối về Dashboard LB ────────────────────────────────────
  const DASHBOARD_HOST = process.env.DASHBOARD_HOST || 'localhost';
  const DASHBOARD_PORT = process.env.DASHBOARD_PORT || 8000;

  // Đếm tổng request đã xử lý (không tính health check, favicon, stats)
  let requestCount = 0;
  const EXCLUDED_PATHS = ['/health', '/favicon.ico', '/robots.txt', '/stats'];

  /**
   * Gửi log request về Load Balancer dashboard
   * Dùng khi request đến trực tiếp (qua AWS ALB thật), không qua Node LB
   */
  function reportToLBDashboard({ clientIp, duration, path }) {
    ...
    req.on('error', () => {}); // Bỏ qua lỗi kết nối (dashboard có thể chưa chạy)
    req.write(payload);
    req.end();
  }
  ...
  // ── HTML template — trang hiển thị khi truy cập EC2 instance ────────────
...
function startLoadTest() {
  ...
  // ── Endpoint chính — trả về trang HTML của instance ─────────────────────
  app.get('/', (req, res) => {
  ...
  // ── Health check — dùng bởi lb-server/healthCheck.js ────────────────────
