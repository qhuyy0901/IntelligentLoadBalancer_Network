...
const app = express();
const PORT = 3002;
const SERVER_ID = 'ec2-2';
const SERVER_NAME = 'EC2-2';
...
.console-body .log-ok{color:#4ade80}.console-body .log-info{color:#60a5fa}.console-body .log-warn{color:#fbbf24}
...
@media(max-width:768px){.grid2{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}.hero h1{font-size:1.5rem}.actions{flex-direction:column}.abtn{width:100%;justify-content:center}}
...
<div class="stats">
  <div class="stat"><div class="stat-icon" style="background:rgba(var(--accent-rgb),.12);color:var(--accent)"><i class="fas fa-server"></i></div><div class="stat-val">${inst.name}</div><div class="stat-lbl">Instance</div></div>
  <div class="stat"><div class="stat-icon" style="background:rgba(59,130,246,.12);color:#60a5fa"><i class="fas fa-exchange-alt"></i></div><div class="stat-val" id="reqCount">${reqCount}</div><div class="stat-lbl">Total Requests</div></div>
  <div class="stat"><div class="stat-icon" style="background:rgba(139,92,246,.12);color:#a78bfa"><i class="fas fa-clock"></i></div><div class="stat-val">${processingTime} ms</div><div class="stat-lbl">Response Time</div></div>
  <div class="stat"><div class="stat-icon" style="background:rgba(245,158,11,.12);color:#fbbf24"><i class="fas fa-globe-asia"></i></div><div class="stat-val" style="font-size:1rem">${inst.region}</div><div class="stat-lbl">Region</div></div>
  <div class="stat"><div class="stat-icon" style="background:rgba(239,68,68,.12);color:#f87171"><i class="fas fa-microchip"></i></div><div class="stat-val">${inst.type}</div><div class="stat-lbl">Instance Type</div></div>
  <div class="stat"><div class="stat-icon" style="background:rgba(34,197,94,.12);color:#4ade80"><i class="fas fa-heart"></i></div><div class="stat-val"><span class="tag tag-green"><i class="fas fa-check-circle"></i> Healthy</span></div><div class="stat-lbl">Health Status</div></div>
</div>
<div class="grid2">
  <div class="card"><div class="card-hdr"><h3><i class="fas fa-info-circle"></i> Instance Details</h3></div><div class="card-body">
  ...
  <div class="card"><div class="card-hdr"><h3><i class="fas fa-shield-alt"></i> Health Check Config</h3></div><div class="card-body">
...
<div class="console"><div class="console-hdr"><div class="dots"><span style="background:#ef4444"></span><span style="background:#f59e0b"></span><span style="background:#22c55e"></span></div>Activity Console</div>
...
<div class="footer"><p>© 2026 Intelligent Load Balancer — University Networking Project | ${inst.name} | All data is simulated, no database</p></div>
<div class="toast-rack" id="toastRack"></div>
<script>
let localReqCount = ${reqCount};
function toast(msg, type) { const rack = document.getElementById('toastRack'); const t = document.createElement('div'); t.className = 'toast toast-' + (type||'info'); t.innerHTML = '<i class="fas fa-' + (type==='ok'?'check-circle':'info-circle') + '"></i> ' + msg; rack.appendChild(t); setTimeout(() => t.remove(), 3000); }
function addLog(text, cls) { const body = document.getElementById('consoleBody'); const line = document.createElement('div'); line.className = 'log-line'; line.innerHTML = '<span class="log-time">' + new Date().toLocaleTimeString() + '</span><span class="' + (cls||'log-info') + '">' + text + '</span>'; body.appendChild(line); body.scrollTop = body.scrollHeight; }
function refreshStatus() { fetch('/health').then(r => r.json()).then(d => { addLog('✓ Health: ' + d.status + ' | Uptime: ' + Math.round(d.uptime) + 's', 'log-ok'); toast('Health check: ' + d.status, 'ok'); }).catch(() => { addLog('✗ Health check failed', 'log-warn'); }); }
function simulateAction() { const actions = ['Cache cleared','Config reloaded','Connections drained','Metrics exported','Logs rotated']; const a = actions[Math.floor(Math.random()*actions.length)]; addLog('⚡ ' + a, 'log-warn'); toast(a, 'ok'); }
function fakeRequest() { const routes = ['/api/users','/api/products','/api/orders','/api/health','/api/cart']; const r = routes[Math.floor(Math.random()*routes.length)]; const lat = Math.floor(Math.random()*120)+15; const code = Math.random()<.9 ? 200 : 500; localReqCount++; document.getElementById('reqCount').textContent = localReqCount; addLog('◆ ' + (code===200?'✓':'✗') + ' ' + r + ' → ' + code + ' (' + lat + 'ms)', code===200?'log-ok':'log-warn'); toast(r + ' → ' + code, code===200?'ok':'info'); }
</script>
</body>
</html>`;
}
